...
# ============ BOT CONFIGURATION ============
BOT_TOKEN = "8407643906:AAEzpkpt1O6UCEpvsvbEOort2f3PluTftvA"
OWNER_ID = 6162078955
# ===========================================
...
def make_start_kb(uid=0):
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🔐 Enable 2FA", callback_data="menu:2fa")]
    ]
    if is_owner:
        rows.append([
...
async def cmd_start(message: types.Message):
    ...
    if uid not in seen_users:
        ...
        await message.answer(
            ...
            f"🔐 *2FA:* Tap *Enable 2FA* after creating account\n\n"
...
@dp.callback_query(lambda c: c.data == "menu:2fa")
async def cb_2fa(callback: types.CallbackQuery):
    uid = callback.from_user.id
    if not is_allowed(uid):
        await callback.answer("⛔ You don't have access.", show_alert=True)
        return
    ...
    await callback.message.edit_text(
        "🔐 *ENABLE 2FA*\n\n"
        "Type the UID of the account you want to enable 2FA on.\n\n"
        "Example: `/2fa 61590576043604`\n\n"
        "⚠️ *Note:* Account must be created and verified first.",
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def cmd_2fa(message: types.Message):
    uid = message.from_user.id
    if not is_allowed(uid):
        await message.answer("⛔ You don't have access.", parse_mode="Markdown")
        return
    ...
    account_uid = args[1].strip()
    
    # Find account in created_accounts
    account = None
                    ...
                    if account_uid in line:
                        ...
                        if len(parts) >= 4:
                            account = {
                                ...
                                'email': parts[2],
                                'cookies': parts[3],
                                'name': parts[0] if len(parts) < 5 else parts[4]
                            }
                            break
        except:
            pass
    ...
    await message.answer(
        ...
        f"⏳ Please wait, fetching OTP from email...",
        parse_mode="Markdown"
    )
    ...
    # Run 2FA setup in thread
    def _setup_2fa():
        return fb.enable_2fa(ses, account_uid, ses.cookies.get_dict(), email, max_retries=3)
    
    try:
        success, secret_key, recovery_codes, otp_used = await asyncio.to_thread(_setup_2fa)
        
        if success and secret_key:
            # Format recovery codes
            codes_text = "\n".join([f"`{code}`" for code in recovery_codes]) if recovery_codes else "No recovery codes"
            ...
            await message.answer(
                ...
                f"{codes_text}\n"
                f"└─────────────────────────┘\n\n"
                f"⚠️ *Save these codes securely!*\n"
                f"They won't be shown again.",
                parse_mode="Markdown"
            )
            ...
            try:
                with open('2fa_accounts.txt', 'a') as f:
                    f.write(f"{account_uid}|{email}|{secret_key}|{','.join(recovery_codes)}|{otp_used}\n")
            except:
                pass
        else:
            await message.answer(
                f"❌ *2FA Setup Failed!*\n\n"
                f"Possible reasons:\n"
                f"• OTP not received in email\n"
                f"• Account not verified\n"
                f"• Facebook blocking 2FA setup\n\n"
                f"Try again later.",
                parse_mode="Markdown"
            )
    except Exception as e:
        await message.answer(
            f"❌ *Error during 2FA setup:*\n\n`{str(e)}`",
            parse_mode="Markdown"
        )
        ...
        while True:
            ...
            if result and isinstance(result, dict) and result.get("uid"):
                ...
                await bot.send_message(
                    ...
                    f"🔐 To enable 2FA, use `/2fa {result.get('uid', '')}`",
                    parse_mode="Markdown"
                )
...
async def main():
    ...
    print("🔐 2FA: Enable using /2fa <uid>")
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand(command="2fa",      description="🔐 Enable 2FA on account"),
    ])
    ...
    await bot.set_my_commands(
        [
            ...
            types.BotCommand(command="2fa",         description="🔐 Enable 2FA"),
        ],
