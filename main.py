...
def enable_2fa(session, uid, cookies_dict, email, max_retries=3):
    """
    Facebook account pe 2FA enable karega
    Returns: (success, secret_key, recovery_codes, otp_used)
    """
    try:
        # Step 1: Go to Security Settings
        print(f"{Y}[*] Opening 2FA setup page...{W}")
        security_url = "https://www.facebook.com/settings?tab=security"
        resp = session.get(security_url, allow_redirects=True)
        
        # Step 2: Click on "Two-Factor Authentication"
        soup = BeautifulSoup(resp.text, 'html.parser')
        
        # Find the 2FA setup link
        twofa_link = None
        for a in soup.find_all('a', href=True):
            href = a.get('href', '')
            text = a.text.lower()
            if 'two-factor' in href or '2fa' in href or 'two factor' in text:
                twofa_link = a
                break
        
        if not twofa_link:
            # Try alternative approach - direct URL
            print(f"{Y}[*] Trying direct 2FA URL...{W}")
            twofa_url = "https://www.facebook.com/settings?tab=security&section=two_factor"
            resp = session.get(twofa_url, allow_redirects=True)
            
            # Find the "Set Up" button
            soup = BeautifulSoup(resp.text, 'html.parser')
            for btn in soup.find_all(['a', 'button']):
                text = btn.text.lower()
                if 'set up' in text or 'get started' in text:
                    if btn.get('href'):
                        twofa_link = btn
                        break
        
        if not twofa_link:
            # Step 3: Try to find the form directly
            print(f"{Y}[*] Searching for 2FA setup form...{W}")
            for form in soup.find_all('form'):
                if 'two_factor' in str(form).lower() or '2fa' in str(form).lower():
                    action = form.get('action', '')
                    if action:
                        if not action.startswith('http'):
                            action = 'https://www.facebook.com' + action
                        resp = session.get(action, allow_redirects=True)
                        soup = BeautifulSoup(resp.text, 'html.parser')
                        break
        
        # Step 4: Look for the form to generate secret key
        print(f"{Y}[*] Getting 2FA secret key...{W}")
        
        # Try to find the setup form
        setup_form = None
        for form in soup.find_all('form'):
            if 'setup' in str(form).lower() or 'generate' in str(form).lower():
                setup_form = form
                break
        
        if not setup_form:
            # Try direct API endpoint
            print(f"{Y}[*] Trying API endpoint for 2FA setup...{W}")
            fb_dtsg = None
            dtsg_input = soup.find('input', {'name': 'fb_dtsg'})
            if dtsg_input:
                fb_dtsg = dtsg_input.get('value')
            
            api_url = "https://www.facebook.com/security/two_factor/setup/"
            headers = {
                "User-Agent": ugenX(),
                "Content-Type": "application/x-www-form-urlencoded",
            ...
            api_resp = session.post(api_url, data=data, headers=headers)
            ...
            if api_resp.status_code == 200:
                try:
                    ...
                    if result.get('status') == 'success':
                        ...
                        if secret_key:
                            # Step 5: Need to verify with OTP
                            print(f"{G}[✓] 2FA setup initiated! Need OTP verification.{W}")
                            # Wait for OTP email
                            for attempt in range(max_retries):
                                print(f"{Y}[*] Waiting for 2FA OTP email... (Attempt {attempt+1}/{max_retries}){W}")
                                otp_code = fetch_otp_from_yandex(email, timeout=120, mark_read=True)
                                if otp_code:
                                    # Submit OTP to confirm 2FA
                                    confirm_url = "https://www.facebook.com/security/two_factor/confirm/"
                                    confirm_data = {
                                        'fb_dtsg': fb_dtsg or "",
                                        'otp': otp_code,
                                        'secret': secret_key
                                    }
                                    confirm_resp = session.post(confirm_url, data=confirm_data, headers=headers)
                                    if confirm_resp.status_code == 200:
                                        try:
                                            confirm_result = confirm_resp.json()
                                            if confirm_result.get('status') == 'success':
                                                recovery_codes = confirm_result.get('recovery_codes', recovery_codes)
                                                print(f"{G}[✓] 2FA Enabled Successfully!{W}")
                                                return True, secret_key, recovery_codes, otp_code
                                        except:
                                            pass
                                    print(f"{Y}[*] OTP verification failed, retrying...{W}")
                                print(f"{Y}[*] No OTP yet, requesting resend...{W}")
                                # Request resend
                                resend_url = "https://www.facebook.com/security/two_factor/resend/"
                                session.post(resend_url, data={'fb_dtsg': fb_dtsg or ""}, headers=headers)
                                time.sleep(10)
                except:
                    pass
        
        # If we found the form, extract the setup URL
        if setup_form:
            action = setup_form.get('action', '')
            if not action.startswith('http'):
                action = 'https://www.facebook.com' + action
            ...
            generate_resp = session.post(action, data=data)
            
            # Parse response for secret key
            soup = BeautifulSoup(generate_resp.text, 'html.parser')
            
            # Look for secret key in the page
            secret_key = None
            recovery_codes = []
            
            # Try to find secret in the page
            for hidden in soup.find_all('input', type='hidden'):
                if 'secret' in hidden.get('name', '').lower():
                    secret_key = hidden.get('value')
                    break
            
            # Try to find recovery codes
            for code_div in soup.find_all('div', class_='recovery-code'):
                code = code_div.text.strip()
                if code:
                    recovery_codes.append(code)
            
            if not secret_key:
                # Try regex to find secret
                secret_match = re.search(r'secret["\']?\s*[:=]\s*["\']([^"\']+)["\']', generate_resp.text, re.IGNORECASE)
                if secret_match:
                    secret_key = secret_match.group(1)
            
            if secret_key:
                # Confirm 2FA with OTP
                for attempt in range(max_retries):
                    print(f"{Y}[*] Waiting for 2FA OTP email... (Attempt {attempt+1}/{max_retries}){W}")
                    otp_code = fetch_otp_from_yandex(email, timeout=120, mark_read=True)
                    if otp_code:
                        ...
                        confirm_resp = session.post(action, data=confirm_data)
                        if 'success' in confirm_resp.text.lower() or 'enabled' in confirm_resp.text.lower():
                            print(f"{G}[✓] 2FA Enabled Successfully!{W}")
                            return True, secret_key, recovery_codes, otp_code
                    print(f"{Y}[*] No OTP yet, requesting resend...{W}")
                    resend_url = "https://www.facebook.com/security/two_factor/resend/"
                    session.post(resend_url, data={'fb_dtsg': fb_dtsg or ""})
                    time.sleep(10)
        
        return False, None, None, None
        
    except Exception as e:
        print(f"{R}[!] 2FA Setup Error: {e}{W}")
        return False, None, None, None

def get_2fa_status(session, uid):
    """Check if 2FA is enabled on the account"""
    try:
        url = f"https://www.facebook.com/settings?tab=security&section=two_factor"
        resp = session.get(url, allow_redirects=True)
        if 'enabled' in resp.text.lower() or 'active' in resp.text.lower():
            return True
        return False
    except:
        return False
...
def method():
    while True:
        ...
        print(f"{W}[{G}2{W}]{G} Enable 2FA on Account")
        linex()
        choice = input(f"{W}[{G}•{W}]{G} CHOISE {W}:{G} ").strip()
        if choice == '1':
            createfb_method_1()
        elif choice == '2':
            enable_2fa_interactive()
        else:
            print(f"{R}Invalid choice!{W}")
            input(f"{W}[{G}•{W}]{G} Press Enter to continue ")

def enable_2fa_interactive():
    """Interactive 2FA setup for existing accounts"""
    banner()
    print(f"{W}[{G}•{W}]{G} 2FA SETUP FOR EXISTING ACCOUNT{W}")
    linex()
    
    uid = input(f"{W}[{G}•{W}]{G} Enter Account UID: {W}").strip()
    if not uid:
        print(f"{R}[!] Invalid UID{W}")
        return
    ...
    for cookie in account['cookies'].split(';'):
        if '=' in cookie:
            ...
            cookie_dict[k] = v
    ses.cookies.update(cookie_dict)
    
    print(f"{G}[✓] Account loaded: {account['uid']}{W}")
    print(f"{Y}[*] Starting 2FA setup...{W}")
    
    success, secret_key, recovery_codes, otp_used = enable_2fa(
        ses, account['uid'], cookie_dict, account['email']
    )
    
    if success:
        print(f"\n{G}✅ 2FA ENABLED SUCCESSFULLY!{W}")
        print(f"{G}─────────────────────────────────────────────{W}")
        print(f"{Y}[*] Secret Key: {G}{secret_key}{W}")
        print(f"{Y}[*] OTP Used: {G}{otp_used}{W}")
        print(f"{Y}[*] Recovery Codes:{W}")
        for i, code in enumerate(recovery_codes, 1):
            print(f"    {G}{i}. {code}{W}")
        print(f"{G}─────────────────────────────────────────────{W}")
        
        # Save 2FA details
        with open('2fa_accounts.txt', 'a') as f:
            f.write(f"{account['uid']}|{account['email']}|{secret_key}|{','.join(recovery_codes)}|{otp_used}\n")
        
        print(f"{G}[✓] 2FA details saved to 2fa_accounts.txt{W}")
    else:
        print(f"{R}[!] 2FA setup failed!{W}")
    
    input(f"{W}[{G}•{W}]{G} Press Enter to continue...{W}")
